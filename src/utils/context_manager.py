...
class ContextManager:
    ...
    def _compress_messages(self, messages: List[BaseMessage]) -> List[BaseMessage]:
        ...
        if self.is_over_limit(compressed):
            ...
            # We want to add messages from the end (newest) to start (oldest)
            # until we hit the limit.
            result_messages = []
            current_tokens = self.count_tokens(preserved_messages)
            ...
            for msg in reversed(remaining_messages):
                ...
                if current_tokens + msg_tokens <= self.token_limit:
                    ...
                    # If the message is too large, try to truncate it to fit
                    remaining_tokens = self.token_limit - current_tokens
                    # Only truncate text content
                    if remaining_tokens > 100 and hasattr(msg, "content") and isinstance(msg.content, str):
                        # Approximate chars (assuming 1 token approx 1-4 chars, be conservative with 1)
                        # But count_text_tokens logic is complex. 
                        # Simple truncation: just take a chunk.
                        # For simplicity and safety, if it's a huge message, we truncate it hard.
                        # But better to just stop if we can't fit a reasonable amount.
                        # Let's try to fit what we can.
                        logger.warning(f"Truncating message of type {type(msg).__name__} to fit token limit")
                        # Heuristic: remaining tokens * 2 chars (avg)
                        chars_to_keep = remaining_tokens * 2
                        msg.content = msg.content[:chars_to_keep] + "... [truncated]"
                        # Recalculate
                        new_tokens = self._count_message_tokens(msg)
                        if current_tokens + new_tokens <= self.token_limit:
                            result_messages.insert(0, msg)
                            current_tokens += new_tokens
                        else:
                             logger.warning(f"Message still too large after truncation, skipping.")
                    
                    break

            compressed = preserved_messages + result_messages
        ...
        try:
            ...
            if isinstance(msg, dict):
                if "content" not in msg:
                    logger.warning(f"Message {i} (dict) has no content key")
                    msg["content"] = ""
