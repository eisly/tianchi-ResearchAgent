...
- **Strictly prohibits fabricating URLs or citation sources. All links must come from search results or provided context.**
...
6. **Key Citations**
   ...
   - **CRITICAL WARNING: Strictly DO NOT fabricate URLs. Every URL must be real and come from the search results. If a source cannot be found, do not invent one.**
...
- **ABSOLUTELY NO FABRICATION OF REFERENCE LINKS (URLs). Any link appearing in the report must be valid and come from previous research steps.**
...
# Final Answer Format
At the very end of the report, you must provide a concise final answer in the following format, containing only the answer itself (noun or number), without any explanation:
[[Final Answer: <Answer>]]
**CRITICAL**: If the user explicitly requested a specific language for the answer (e.g., "in English", "English name"), you MUST provide the <Answer> in that language, even if the rest of the report is in a different language.
